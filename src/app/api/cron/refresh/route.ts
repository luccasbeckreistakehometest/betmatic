...
import { runGameLearningJob } from "@/lib/ledger/game-learn";
import { runRevertCheck } from "@/lib/ledger/revert";
 ...
 *   learn-game — the closed loop: rolls back any applied prompt version already measured worse than
 *             the one it replaced (zero tokens), then runs a post-mortem on each game whose tickets
 *             have all settled and that has not been read yet. Every finding becomes a row in the
 *             admin's approval queue; nothing reaches the live prompt without a click. Called every
 *             tick, gated per game on job_runs. LEARN_PER_GAME=0 switches it off without a deploy.
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (job === "learn-game") {
      // The loop's own tick. Two things, in this order and for this reason: the reversion check is
      // arithmetic over the ledger and costs nothing, so a version that is already measured worse
      // comes back BEFORE another night's tickets are written under it; then the games that finished
      // settling and have not been read yet get their post-mortem, up to the per-tick cap and never
      // past the day's AI ceiling. Every outcome is a row waiting for an admin — nothing here changes
      // the live prompt.
      const reverts = runRevertCheck();
      const force = url.searchParams.get("gameId") ?? undefined;
      const max = Number(url.searchParams.get("max") ?? "");
      const result = await runGameLearningJob({ force, max: Number.isFinite(max) && max > 0 ? max : undefined });
      logEvent("job.learn-game", { ...result, rows: undefined, reverted: reverts.reverted, checked: reverts.checked, ms: Date.now() - started });
      return NextResponse.json({ job, ...result, reverts });
    }
    if (job === "evaluate") {

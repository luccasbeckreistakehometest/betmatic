...
const C = {
  pt: { eyebrow: "Prova pública", title: "Todos os bilhetes. Nenhum escondido.", sub: "Cada bilhete que o Betmatic gera fica registrado no momento em que nasce, aparece aqui quando o jogo começa e é liquidado sozinho contra o placar real. Sem seleção, sem editar depois. Se um dia ficar feio, vai ficar feio aqui também.",
    ...
    liveFoot: "O retorno só entra quando o preço era pegável. Quando alguma casa brasileira cota a linha com o jogo rolando, a gente lê esse preço e o bilhete sai com ele: esse fica na linha \"ao vivo (preço ao vivo)\" de Por origem, e só essa linha mostra retorno. Quando nenhuma casa cota a linha em jogo, o preço que temos é o da tabela de antes do jogo, e no 3º quarto ele já não existe: esse bilhete não tem retorno, só o que dá para conferir — com que frequência a leitura acerta, contra a chance que ela mesma prometeu.",
    ...
    balanceTitle: "Balanço (pré-jogo)", balanceSub: "Uma unidade em cada bilhete de pré-jogo decidido. Leitura ao vivo não entra aqui: esta tabela é o registro do pré-jogo, e mexer no que ela conta mudaria o que o número quer dizer. O ao vivo aparece em Por origem, separado em preço ao vivo (com ROI) e preço de referência (só acerto). O dia é o de Brasília.", overall: "Geral", today: "Hoje", dayCol: "Dia", ticketsCol: "Bilhetes", hitsCol: "Acertos", stakedCol: "Apostado", returnedCol: "Retorno", roiCol: "ROI", liveCol: "Ao vivo (acerto · desvio)", noToday: "Nenhum bilhete decidido hoje ainda.", byScope: "Por origem", scopePregame: "Pré-jogo", scopeLive: "Ao vivo (preço de referência)", scopeLivePriced: "Ao vivo (preço ao vivo)", liveTag: "ao vivo", methodTitle: "Como medimos", methodBody: "Todo bilhete é salvo no momento em que é gerado, com as odds e a chance estimada, e fica visível para todo mundo assim que a bola rola. Quando o jogo termina, cada linha é conferida contra o placar e as estatísticas oficiais: se não dá para conferir com certeza, a linha é anulada — nunca chutada. O ROI do topo considera 1 unidade apostada em cada bilhete de pré-jogo decidido. Leitura ao vivo só tem ROI quando todas as linhas do bilhete saíram por um preço que alguma casa brasileira estava cotando com o jogo rolando, lido segundos antes; quando não tinha esse preço, o bilhete é conferido igual mas não gera retorno nenhum.", window: "O registro conta a partir de {d}, quando a plataforma passou a gerar os bilhetes sozinha. O que veio antes era outro produto, montado à mão e em outro esporte, e misturar os dois descreveria uma coisa que ninguém pode comprar." },
  en: { eyebrow: "Public track record", title: "Every ticket. None hidden.", sub: "Every ticket Betmatic generates is logged the moment it is born, shows up here once its game kicks off, and is graded automatically against the real score. No curation, no edits after the fact. If it ever looks bad, it looks bad here too.",
    ...
    liveFoot: "A return appears only where the price was takeable. When a Brazilian book quotes the line with the game under way we read that price and the ticket is written at it: those sit in the \"live (in-play price)\" row of By origin, and only that row shows a return. When no book quotes the line in play, the only price we have is the pre-game board's, and by the third quarter it no longer exists: that ticket has no return, only what can be checked — how often the read lands, against the chance it gave itself.",
    ...
    balanceTitle: "Balance (pre-game)", balanceSub: "One unit on every decided pre-game ticket. Live reads stay out of this table: it is the pre-game record, and widening what it counts would change what the number means. Live sits in By origin, split into in-play price (with an ROI) and reference price (hit rate only). Days are Brasília days.", overall: "Overall", today: "Today", dayCol: "Day", ticketsCol: "Tickets", hitsCol: "Hits", stakedCol: "Staked", returnedCol: "Returned", roiCol: "ROI", liveCol: "Live (hit · gap)", noToday: "No ticket decided today yet.", byScope: "By origin", scopePregame: "Pre-game", scopeLive: "Live (reference price)", scopeLivePriced: "Live (in-play price)", liveTag: "live", methodTitle: "How we measure", methodBody: "Every ticket is saved the moment it is generated, with its odds and modelled probability, and becomes visible to everyone at kickoff. When the game ends, each leg is checked against the official score and stats: if it cannot be graded with certainty, the leg is voided — never guessed. The headline ROI assumes 1 unit staked on every decided pre-game ticket. A live read carries an ROI only when every leg was written at a price a Brazilian book was quoting with the game under way, read seconds earlier; without that price the ticket is graded the same way and returns nothing at all.", window: "The record counts from {d}, when the platform began generating the tickets on its own. What came before was another product, built by hand on another sport, and averaging the two would describe something nobody can buy." },
};
      ...
      <section className="text-fg">
        ...
        {publish && s.settled > 0 && (
          <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            {[[c.byMarket, s.byMarket], [c.bySport, s.bySport], [c.byBand, s.byBand], [c.byScope, s.byScope.map((r) => ({ ...r, key: r.key === "live_priced" ? c.scopeLivePriced : r.key === "live" ? c.scopeLive : c.scopePregame }))]].map(([title, rows]) => (

...
const entry = (legs: { predicted: number; computed?: number; raw?: number; outcome: "won" | "lost" | "void" }[]): LedgerEntry => ({
  ...
  legs: legs.map((l) => ({ selection: "s", market: "player_prop", sourceBasis: "measured history", predictedProbability: l.predicted, computedProbability: l.computed, rawProbability: l.raw, oddsDecimal: 1.9, outcome: l.outcome })),
});
...
describe("the computed model against the language model", () => {
  ...
  it("races the computed number against the model's RAW estimate, not the anchored one it was served with", () => {
    // Anchored to within 8 points of computed, the served number can never differ much; the raw one can.
    const race = modelRace([entry([{ predicted: 0.58, computed: 0.5, raw: 0.9, outcome: "lost" }, { predicted: 0.62, computed: 0.7, raw: 0.3, outcome: "won" }])]);
    expect(race.averageRaw).toBeCloseTo(0.6, 6);
    expect(race.averagePredicted).toBeCloseTo(0.6, 6);
    expect(race.brierRaw).toBeCloseTo(((0.9 - 0) ** 2 + (0.3 - 1) ** 2) / 2, 6);
    expect(race.brierPredicted).toBeCloseTo(((0.58 - 0) ** 2 + (0.62 - 1) ** 2) / 2, 6);
    expect(race.brierComputed).toBeLessThan(race.brierRaw);
    // A leg logged before the raw number existed falls back to the served number.
    expect(modelRace([entry([{ predicted: 0.58, computed: 0.5, outcome: "lost" }])]).averageRaw).toBeCloseTo(0.58, 6);
  });
  ...
  it("stays silent until ten legs have settled, then says which guide has been better", () => {
    ...
    const many = Array.from({ length: 12 }, (_, i) => entry([{ predicted: 0.68, computed: 0.6, raw: 0.9, outcome: i % 2 ? "won" : "lost" }]));
    const line = modelRaceLine(modelRace(many));
    expect(line).toMatch(/COMPUTED MODEL vs YOUR ESTIMATES \(12 settled legs, 50% won\)/);
    expect(line).toMatch(/your unaided estimates averaged 90%/);
    expect(line).toMatch(/the anchored numbers served 68%/);

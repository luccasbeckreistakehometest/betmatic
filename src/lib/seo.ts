...
/** The site-wide share card (src/app/og-default.png/route.tsx); pages with their own card override it. */
export const DEFAULT_OG_IMAGE = { url: "/og-default.png", width: 1200, height: 630, alt: "Betmatic" };
...
export function pageMetadata(input: { lang: Lang; title: string; description: string; paths: Record<Lang, string>; index?: boolean; image?: string }): Metadata {
  ...
  return {
    ...
    openGraph: {
      ...
      type: "website",
      images: [input.image ? { url: input.image } : DEFAULT_OG_IMAGE],
    },
    twitter: { card: "summary_large_image", title, description, images: [input.image ?? DEFAULT_OG_IMAGE.url] },

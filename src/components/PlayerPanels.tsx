      ...
      {market.posted.map((p, i) => {
        ...
        return (
          <li key={i} className="flex flex-wrap items-center gap-x-3 gap-y-1 py-1.5 text-tiny">
            ...
            <button type="button" onClick={() => onPick(p.line, p.side)} className="u-hit ml-auto rounded-control border border-line-control px-2 py-0.5 text-label text-fg-muted transition-colors duration-(--dur-1) hover:bg-surface-2 hover:text-fg max-md:min-h-9 max-md:px-3">{c.useLine}</button>
          </li>
        );
      })}
    </ul>

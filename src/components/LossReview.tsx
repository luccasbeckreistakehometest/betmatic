...
export function LossReview({ slug, lang, compact = false }: { slug: string; lang: Lang; compact?: boolean }) {
  ...
  if (state === "idle") return <button onClick={() => void ask()} className={`rounded-control border border-line-control px-3 py-1.5 text-fg-muted hover:bg-surface-2 hover:text-fg ${compact ? "text-label" : "text-sm"}`} data-testid="why-lost">{t("whyLost")}</button>;

...
const List = ({ title, items }: { title: string; items?: string[] }) => items?.length ? (
  <div><p className="text-micro uppercase tracking-wider text-fg-dim">{title}</p><ul className="mt-1 space-y-1">{items.map((t) => <li key={t} className="text-fg-muted">· {t}</li>)}</ul></div>
) : null;

...
export default async function globalSetup() {
  ...
  const startsAt = (JSON.parse(fs.readFileSync(path.join(dir, "espn-fixtures", "world.json"), "utf8")) as { games?: { id: string; startsAt: string }[] }).games?.find((g) => g.id === "990000101")?.startsAt ?? new Date(Date.now() + 5 * 3_600_000).toISOString();

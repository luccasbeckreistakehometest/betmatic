...
| `books` | a cada tick | preços das casas brasileiras (Superbet, KTO, EstrelaBet e as outras da Altenar, Betfair Exchange, Sportingbet, Betnacional) para todo jogo das próximas 48 h, só pré-jogo; cada casa tem timeout e erro isolados; `BR_BOOKS` escolhe quais rodam e o painel admin liga/desliga cada uma; `pnpm tsx scripts/books-run.mts wnba` roda o mesmo job no shell |
